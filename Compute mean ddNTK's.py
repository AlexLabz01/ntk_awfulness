...
def compute_mean_dd_NTKs(width, R, S, T, U):

    num_inpts = R.shape[0]
    I = torch.eye(num_inpts, dtype=R.dtype, device=R.device)

    first = torch.einsum('ab, cd, abcd->abcd', I, I, R)

    second = first.permute(0, 2, 3, 1)

    third = first.permute(0, 3, 1, 2)

    mean_dd_I_NTK = (1/width) * (first + second + third)

    first = torch.einsum('ab, cd, abcd->abcd', I, I, S)

    second = torch.einsum('ab, cd, abcd->abcd', I, I, T)
    second = second.permute(0, 2, 3, 1)

    third = torch.einsum('ab, cd, abcd->abcd', I, I, U)
    third = third.permute(0, 3, 1, 2)

    mean_dd_II_NTK = (1/width) * (first + second + third)

    return mean_dd_I_NTK, mean_dd_II_NTK
...
if __name__ == "__main__":
    ...
    # the configs to run, supports multiple for killarney runs
    CONFIGS = [
        {"N_train": 3, "N_test": 5,  "hidden_dim": 30,  "depth": 2}
    ]
    ...
    mean_dd_I, mean_dd_II = None, None
    def run_one_config(config, config_id):
        ...
        # Building and training and testing inputs on the unit circle
        ...
        mean_dd_I, mean_dd_II = compute_mean_dd_NTKs(width, R_prev, S_prev, T_prev, U_prev)
